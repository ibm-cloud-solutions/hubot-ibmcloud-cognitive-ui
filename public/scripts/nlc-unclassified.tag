	...
	<script>
		...
		self.loadUnclassified = function() {
			if (data.length > 0){
				...
				util.getDBData('/api/favorites/unclassified', self.observable, 1, limit).then(function(res) {
					data = res.data;
		...
		self.observable.on('page', function(page){
			if (self.showTable) {
				...
				self.observable.trigger('startSpinning');
				util.getDBData(`/api/favorites/unclassified`, self.observable, page, limit).then(function(res) {
					data = res.data;
					self.observable.trigger('stopSpinning');
					self.update({
						showTable: true,
